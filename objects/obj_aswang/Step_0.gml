...
if(sqrt(sqr(x - Obj_Junjun.x) + sqr(y - Obj_Junjun.y)) <= combat_distance*2){
	if(sqrt(sqr(x - Obj_Junjun.x) + sqr(y - Obj_Junjun.y)) <= combat_distance){
		state = 2;
		i++;
	}
}
else if(sqrt(sqr(x - Obj_Junjun.x) + sqr(y - Obj_Junjun.y)) <= 540){
	state = 1;
}
else{
	state = 0;
...
switch state{
	...
	case 2: //combat state
		if(i == 180){
			x += lengthdir_x(2*distance_to_point(Obj_Junjun.x, Obj_Junjun.y), point_direction(x, y, Obj_Junjun.x, Obj_Junjun.y));
			y += lengthdir_y(2*distance_to_point(Obj_Junjun.x, Obj_Junjun.y), point_direction(x, y, Obj_Junjun.x, Obj_Junjun.y));
			i = 0;
		}
		break;
}